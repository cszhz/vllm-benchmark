...
async def run_benchmark(num_requests, concurrency, request_timeout, output_tokens, vllm_url, api_key, use_long_context, node_number):
    ...
    return {
        ...
        "latency": {
            ...
            "p50": round(latency_percentiles[0],3),
            "p95": round(latency_percentiles[1],3),
            "p99": round(latency_percentiles[2],3)
        },
        "tokens_per_second": {
            "average": round(avg_tokens_per_second,1)*node_number,
            "p50": round(tps_percentiles[0],1)*node_number,
            "p95": round(tps_percentiles[1],1)*node_number,
            "p99": round(tps_percentiles[2],1)*node_number
        },
        "time_to_first_token": {
            "average": round(avg_ttft,3),
            "p50": round(ttft_percentiles[0],3),
            "p95": round(ttft_percentiles[1],3),
            "p99": round(ttft_percentiles[2],3)
        }
    }
...
def convert_table(results):
    html=convert(results)
    #print(html)
...
if __name__ == "__main__":
    ...
    print(args.node_number)
    results = asyncio.run(run_benchmark(args.num_requests, args.concurrency, args.request_timeout, args.output_tokens, args.vllm_url, args.api_key, args.use_long_context, args.node_number))
    print_results(results)
    convert_table(results)
else:
